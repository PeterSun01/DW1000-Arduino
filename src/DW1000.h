...
//定义基站短地址
#define A0_ADDR     0X8000
#define A1_ADDR     0X8001
#define A2_ADDR     0X8002
#define A3_ADDR     0X8003
...
#define FC_POLL         0X80
#define FC_RESP         0X81
#define FC_FINAL        0X82
#define FC_REPORT       0X83
#define FC_RANGEDATA    0X84
...
//定义拨码开关 GPIO
#define SWITCH_SW1  35
#define SWITCH_SW2  32
#define SWITCH_SW3  33
#define SWITCH_SW4  25
...
public:
	...
	*/
	...
	static void setSNIFFMode(int enable, uint8_t timeOn, uint8_t timeOff);
	...
	*/
	...
	static void setReceiveFrameWaitTimeoutPeriod(uint16_t timeMicroSeconds); 
	...
	*/
	...
	static void getNetworkId(byte id[]);
	static void getDeviceAddress(byte address[]);
	...
	*/
	...
	static void setDefaults_noFrameFilter();
	...
	static double correctRange(double range); 
	static double dwt_getrangebias(uint8_t chan, float range, uint8_t prf);

	static void LedCtl(uint16_t dev_addr);
	static uint16_t ReadSwitch(uint8_t role);
	...
	preamble length). */
	static constexpr byte MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024};
	...
	static constexpr byte MODE_LONGDATA_RANGE_ACCURACY[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024};
