...
// SPI settings
#ifdef ESP32
	// default ESP32 frequency is 80 Mhz, thus divide by 4 is 20 MHz
...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	...
	delay(5);
	// Configure the IRQ pin as INPUT. Required for correct interrupt setting for ESP32
    pinMode(irq, INPUT);
	// start SPI
	SPI.begin();
#ifndef ESP32
	SPI.usingInterrupt(digitalPinToInterrupt(irq)); // not every board support this, e.g. ESP32
#endif
	...
	// attach interrupt
	//attachInterrupt(_irq, DW1000Class::handleInterrupt, CHANGE); // todo interrupt for ESP32
	// TODO throw error if pin is not a interrupt pin
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000Class::handleInterrupt, RISING); // todo interrupt for ESP32
}
...
void DW1000Class::setLedBlink() 
{
	byte gpiomode[LEN_GPIO_MODE];
	memset(gpiomode, 0, LEN_GPIO_MODE);
	readBytes(GPIO_CTRL, GPIO_MODE_SUB, gpiomode, LEN_GPIO_MODE);
	
	//bit 11.10=01   RX_LED ON
	setBit(gpiomode, LEN_GPIO_MODE, 11, 0);
	setBit(gpiomode, LEN_GPIO_MODE, 10, 1);
	//bit 13.12=01   TX_LED ON
	setBit(gpiomode, LEN_GPIO_MODE, 13, 0);
	setBit(gpiomode, LEN_GPIO_MODE, 12, 1);
	writeBytes(GPIO_CTRL, GPIO_MODE_SUB, gpiomode, LEN_GPIO_MODE);

	memset(gpiomode, 0, LEN_GPIO_MODE);
	readBytes(PMSC, PMSC_CTRL0_SUB, gpiomode, LEN_PMSC_CTRL0);
	setBit(gpiomode, LEN_PMSC_CTRL0, 18, 1);
	setBit(gpiomode, LEN_PMSC_CTRL0, 23, 1);
	writeBytes(PMSC, PMSC_CTRL0_SUB, gpiomode, LEN_PMSC_CTRL0);
...
void DW1000Class::setPaLnaIO() 
{
	byte gpiomode[LEN_GPIO_MODE];
	memset(gpiomode, 0, LEN_GPIO_MODE);
	readBytes(GPIO_CTRL, GPIO_MODE_SUB, gpiomode, LEN_GPIO_MODE);
	
	//bit 15.14=01   MSGP4   PA_ON
	setBit(gpiomode, LEN_GPIO_MODE, 15, 0);
	setBit(gpiomode, LEN_GPIO_MODE, 14, 1);
	//bit 17.16=01   MSGP5   EXT_TXE
	setBit(gpiomode, LEN_GPIO_MODE, 17, 0);
	setBit(gpiomode, LEN_GPIO_MODE, 16, 1);
	//bit 19.18=01   MSGP6   EXT_RXE
	setBit(gpiomode, LEN_GPIO_MODE, 19, 0);
	setBit(gpiomode, LEN_GPIO_MODE, 18, 1);
...
void DW1000Class::setSNIFFMode(int enable, uint8_t timeOn, uint8_t timeOff)
{
	uint16_t sniff_reg = (((uint16_t)timeOff << 8) | timeOn) & RX_SNIFF_MASK;
	byte sniffmode[2];
	byte pmsc_reg[LEN_PMSC_CTRL0];

	if(enable==1)
	{
		writeValueToBytes(sniffmode,sniff_reg,2);
		writeBytes(RX_SNIFF_ID, RX_SNIFF_OFFSET, sniffmode, 2);
		
		memset(pmsc_reg, 0, LEN_PMSC_CTRL0);
		readBytes(PMSC, PMSC_CTRL0_SUB, pmsc_reg, LEN_PMSC_CTRL0);
		setBit(pmsc_reg, LEN_PMSC_CTRL0, 24, 1);
		writeBytes(PMSC, PMSC_CTRL0_SUB, pmsc_reg, LEN_PMSC_CTRL0);
	}
	else if(enable==0)
	{
		memset(sniffmode, 0, 2);
		writeBytes(RX_SNIFF_ID, RX_SNIFF_OFFSET, sniffmode, 2);
		readBytes(PMSC, PMSC_CTRL0_SUB, pmsc_reg, LEN_PMSC_CTRL0);
		setBit(pmsc_reg, LEN_PMSC_CTRL0, 24, 0);
		writeBytes(PMSC, PMSC_CTRL0_SUB, pmsc_reg, LEN_PMSC_CTRL0);
	}
}
...
void DW1000Class::setReceiveFrameWaitTimeoutPeriod(uint16_t timeMicroSeconds) 
{
	if (timeMicroSeconds > 0) 
	{
		byte rx_wfto[LEN_RX_WFTO];
		writeValueToBytes(rx_wfto, timeMicroSeconds, LEN_RX_WFTO);
		writeBytes(RX_WFTO, NO_SUB, rx_wfto, LEN_RX_WFTO);
	...
	if(_channel == CHANNEL_1 || _channel == CHANNEL_2) {
		...
		if(_pulseFrequency == TX_PULSE_FREQ_16MHZ) {
			if(_smartPower) {
				...
				writeValueToBytes(txpower, 0x3E3E3E3E, LEN_TX_POWER);
...
void DW1000Class::getNetworkId(byte id[]) {
	readNetworkIdAndDeviceAddress();
	id[0] = _networkAndAddress[2];
	id[1] = _networkAndAddress[3];
}

void DW1000Class::getDeviceAddress(byte address[]) {
	readNetworkIdAndDeviceAddress();
...
void DW1000Class::commitConfiguration() {
	...
	{
		_antennaDelay.setTimestamp(16456);//16436
		_antennaCalibrated = true;
...
void DW1000Class::setDefaults() {
	if(_deviceMode == TX_MODE) {
		...
		*/
		...
		setReceiverAutoReenable(true);
    	//receivePermanently(true);
		enableMode(MODE_LONGDATA_RANGE_LOWPOWER);

		setLedBlink();
		setPaLnaIO();
		setSNIFFMode(1,1,128);
...
void DW1000Class::setDefaults_noFrameFilter() 
{
...
double DW1000Class::dwt_getrangebias(uint8_t chan, float range, uint8_t prf)
{
...
double DW1000Class::correctRange(double range) 
{
	double distance;
	double distance_to_correct;
	distance=range;
	uint8_t ChannelInt8=0;
	uint8_t FreqInt8=0;
	...
	if(_pulseFrequency == TX_PULSE_FREQ_16MHZ)
	{
		FreqInt8=DWT_PRF_16M;
	}
	else if(_pulseFrequency == TX_PULSE_FREQ_64MHZ)
	{
		FreqInt8=DWT_PRF_64M;
	}
	...
	distance = distance - dwt_getrangebias(ChannelInt8, (float) distance_to_correct, FreqInt8);
	
	if ((distance < 0) || (distance > 200.000))    // discard any results less than <0 cm or >20 km
		distance=0;
...
void DW1000Class::LedCtl(uint16_t dev_addr)
{
  pinMode(PIN_LED1,OUTPUT);     
  pinMode(PIN_LED2,OUTPUT);
  pinMode(PIN_LED3,OUTPUT);
  
  if(dev_addr==A0_ADDR)//A0-yellow
  {
    digitalWrite(PIN_LED1,LOW);//red
    digitalWrite(PIN_LED2,HIGH);//blue
    digitalWrite(PIN_LED3,LOW); //green
  }
  else if(dev_addr==A1_ADDR)//A1-blue
  {
    digitalWrite(PIN_LED1,HIGH);
    digitalWrite(PIN_LED2,LOW);
    digitalWrite(PIN_LED3,HIGH);    
  }
  else if(dev_addr==A2_ADDR)//A2-green
  {
    digitalWrite(PIN_LED1,HIGH);
    digitalWrite(PIN_LED2,HIGH);
    digitalWrite(PIN_LED3,LOW);    
  }
  else if(dev_addr==A3_ADDR)//A3-purple
  {
    digitalWrite(PIN_LED1,LOW);
    digitalWrite(PIN_LED2,LOW);
    digitalWrite(PIN_LED3,HIGH);  
  }
  else if(dev_addr==T0_ADDR)//T0-cyan
  {
    digitalWrite(PIN_LED1,HIGH);
    digitalWrite(PIN_LED2,LOW);
    digitalWrite(PIN_LED3,LOW);    
...
uint16_t DW1000Class::ReadSwitch(uint8_t role)  
{
  uint8_t sw_role=0;
  uint8_t sw_addr1,sw_addr2,sw_addr3;
  
  pinMode(SWITCH_SW1,INPUT_PULLUP);
  pinMode(SWITCH_SW2,INPUT_PULLUP);
  pinMode(SWITCH_SW3,INPUT_PULLUP);
  pinMode(SWITCH_SW4,INPUT_PULLUP);
  sw_role=digitalRead(SWITCH_SW1);
  if(sw_role!=role)
  {
	LedCtl(ERR_ADDR);
    return ERR_ADDR;
  ...
  else if(sw_role==ROLE_ANCHOR)
  {
    ...
    else if((sw_addr1==1)&&(sw_addr2==0)&&(sw_addr3==0))
    {
      LedCtl(A3_ADDR);
      return A3_ADDR;
    }
